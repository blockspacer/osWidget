...

{
    ...
    /** QRect myQRect = QRect(QPoint(startPos,height+3), ...)
        il secondo parametro di QPoint setta l'altezza dalla quale la barra viene disegnata
        QPoint(startPos, height - VALUE);
*/
    //Lets build the backaground rectangle
    //    QRect myQRect = QRect(QPoint(startPos,3) , QSize(pixToCover-1,height-6));

    QRect myQRect = QRect(QPoint(startPos,height-value) , QSize(1,value));
    //std::cout << "height: "<< height << std::endl;

    QRect my2QRect = QRect(QPoint(startPos+10,height-30) , QSize(1,30));

    //Lets build the gradient
    QLinearGradient gradient(myQRect.bottomLeft(), myQRect.bottomRight()); // diagonal gradient from top-left to bottom-right
